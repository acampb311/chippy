...
struct ContentView : View {
    ...
    var body: some View {
        let romView = RomView(currentChip: self.$chip8)
        NavigationView {
            ScrollView {
                romView
                ...
                Button("Step") {
                    ...
                    let instruction = UInt32((UInt32(self.chip8.ram[Int(self.chip8.PC)+0]) << 8) |
    ...
    struct RomView: View {
                ...
                Button("select File")
                {
                    ...
                    if panel.runModal() == .OK {
                        ...
                        if let content = NSData(contentsOf: panel.url!) {
                            self.currentChip.loadRom(rom: [UInt8](content))
                        }
                    }
                }
            }
        ...
        public init(currentChip: Binding<Chip8>)
        {
            self._currentChip = currentChip
        }
    }
